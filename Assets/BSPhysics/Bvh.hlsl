...
inline bool LineTriangleIntersection(BvhTriangle tri, float3 origin, float3 rayStep, out float rayScale,bool ignoreBack)
{
    rayScale = BVH_FLT_MAX;

    float3 normal = tri.normal;
    float dirDot = dot(normal, rayStep);
    if ( dirDot > 0 && ignoreBack) return false;//ray direction same as the normal

    float3 origin_from_pos0 = origin - tri.pos0;
    if(dot(origin_from_pos0, normal) < 0 && ignoreBack) return false;//origin at back of face
...
bool LineAABBIntersection(float3 origin, float3 rayStep, BvhData data)
{
    ...
    for(int axis = 0; axis<3; ++axis)
    {
        ...
        {
            //if ( originOnAxis < minOnAxis - FLOAT_EPSILON || maxOnAxis < originOnAxis + FLOAT_EPSILON ) 
            return false;
        }
        else
        {
            ...
            tFar  = min(tFar, tMax);
            //this line is checking if ray will intersect with aabb in this frame,not correct in this
